...
1. **`backend/.env`** (nếu có) — tạo từ `backend/.env.example` nếu muốn env riêng cho backend.
2. **`.env` ở root project** — dùng chung với frontend/script.

Chỉ cần **một trong hai**; nếu không có `backend/.env` thì mọi thứ lấy từ root `.env`. Cập nhật file `.env` ở root project (hoặc `backend/.env`):
