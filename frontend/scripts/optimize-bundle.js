...
const BUDGETS = {
  javascript: 4 * 1024 * 1024, // 4 MB (Vite uncompressed; antd alone ~1.1 MB)
  css: 200 * 1024, // 200 KB
  images: 500 * 1024, // 500 KB
  fonts: 100 * 1024, // 100 KB
  total: 20 * 1024 * 1024, // 20 MB (includes sourcemaps)
};
...
// Returns total size of files matching `exts` in dirPath. Excludes .map/.gz/.br by default.
function getAssetsSizeByExt(
  dirPath,
  exts = [],
  excludeExts = [".map", ".gz", ".br"]
) {
  let totalSize = 0;
  ...
  files.forEach((file) => {
    ...
    if (stat.isDirectory()) {
      totalSize += getAssetsSizeByExt(filePath, exts, excludeExts);
    } else {
      const lowerFile = file.toLowerCase();
      const excluded = excludeExts.some((ext) => lowerFile.endsWith(ext));
      const included =
        exts.length === 0 || exts.some((ext) => lowerFile.endsWith(ext));
...
function analyzeBundle(buildDir = "build") {
  ...
  // Vite outputs all assets to build/assets/ (JS, CSS, images, fonts)
  const assetsDir = path.join(buildDir, "assets");

  // Tính tổng kích thước theo extension (bỏ qua .map, .gz, .br)
  const jsSize = getAssetsSizeByExt(assetsDir, [".js"]);
  const cssSize = getAssetsSizeByExt(assetsDir, [".css"]);
  const imageSize = getAssetsSizeByExt(
    assetsDir,
    [".png", ".jpg", ".jpeg", ".gif", ".svg", ".webp"],
    []
  );
  const fontSize = getAssetsSizeByExt(
    assetsDir,
    [".woff", ".woff2", ".ttf", ".eot"],
    []
  );
