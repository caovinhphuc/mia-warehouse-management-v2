...
const checkEnvironmentVariables = () => {
  ...
  const requiredVars = [
    ...
    },
    {
      ...
      valid: isConfigured(process.env.GOOGLE_PRIVATE_KEY, hasGooglePrivateKey),
    },
    {
  ...
  const optionalVars = [
    {
      ...
      valid: isConfigured(process.env.REACT_APP_GOOGLE_MAPS_API_KEY),
    },
    {
      key: "TELEGRAM_BOT_TOKEN",
      valid: isConfigured(process.env.TELEGRAM_BOT_TOKEN, isTelegramToken),
    },
    {
      key: "TELEGRAM_CHAT_ID",
      valid: isConfigured(process.env.TELEGRAM_CHAT_ID),
    },
    {
      key: "SENDGRID_API_KEY",
      valid: isConfigured(process.env.SENDGRID_API_KEY),
    },
    {
      key: "EMAIL_FROM",
      valid: isConfigured(process.env.EMAIL_FROM, isProbablyEmail),
    },
    {
      key: "SMTP_USER",
      valid: isConfigured(process.env.SMTP_USER),
    },
    {
      key: "SMTP_PASS",
      valid: isConfigured(process.env.SMTP_PASS),
    },
    {
      key: "REDIS_URL",
      valid: isConfigured(process.env.REDIS_URL),
    },
  ];
  ...
  const missingRequired = requiredVars
    ...
    .map((entry) => entry.key);
  const missingOptional = optionalVars
    .filter((entry) => !entry.valid)
    .map((entry) => entry.key);
  ...
  if (missingRequired.length > 0) {
    addResult(
      "environment",
      "warning",
      `Live integration checks skipped. Missing or placeholder configuration: ${missingRequired.join(", ")}`
    );
    return false;
  }
